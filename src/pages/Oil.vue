<template>
    <div>
        ...
        <group style="padding:58px 0px 0" :title="`今天是${new Date().getDate()}号,油卡充值优惠${oilRatio}%`">
            ...
            <cell :is-link="false" inline-desc='油卡充值有什么优惠规则?' @click.native="$router.push('/money-tip/8')">
            </cell>
        </group>
...
export default {
  ...
  methods: {
    handleOk () {
      ...
      if (!this.formBean.cardno) {
        this.$vux.toast.text('输入充值卡号')
        return
      }
      if (!this.type.length) {
